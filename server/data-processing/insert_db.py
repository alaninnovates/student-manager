                ...
                if student["name"] not in [s["name"] for s in student_metadata]:
                    student_metadata.append(
                        {
                            ...
                            "grade": (
                                student["grade"]
                                if student["grade"] not in (None, float("nan"))
                                else 0
                            ),
                            "parent_cells": (
                                [str(student["parent_cells"])]
                                if not pd.isna(student["parent_cells"])
                                else []
                            ),
                            "parent_emails": (
                                [student["parent_email"]]
                                if not pd.isna(student["parent_email"])
                                else []
                            ),
                            "organization": 1,
                        }
                    ...
                    for s in student_metadata:
                        if s["name"] == student["name"]:
                            if str(student["parent_cells"]) not in s[
                                "parent_cells"
                            ] and not pd.isna(student["parent_cells"]):
                                s["parent_cells"].append(str(student["parent_cells"]))
                            if student["parent_email"] not in s[
                                "parent_emails"
                            ] and not pd.isna(student["parent_email"]):
...
def insert_attendance():
    ...
    for file in files:
        with open(f"out/{file}", "r") as f:
            ...
            for student in data["students"]:
                student_id = next(
                    (s["id"] for s in students if s["name"] == student["name"]), None
                )
                if student_id is None or class_id is None:
                    print(
                        "Error: Student or class not found for",
                        student["name"],
                        "in",
                        data["class_name"],
                    )
